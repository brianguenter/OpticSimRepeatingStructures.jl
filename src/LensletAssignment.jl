...
julia> hex12RGB()
ClusterWithProperties{12, 2, Float64, OpticSim.RGBCluster}(LatticeCluster{12, 2, Float64, 
...
"""
function eyebox_number(tilecoords::NTuple{2,Int64}, cluster::ClusterWithProperties{A,B,C,RGBCluster}, num_eyeboxes::Integer)::Int64 where {A,B,C}
    _, tile_index = cluster_coordinates_from_tile_coordinates(cluster, tilecoords)
    rindices, gindices, bindices = rgbtileindices(cluster)
...
function systemparameters()
    return (eye_box=(10.0mm, 9.0mm), fov=(90.0°, 60.0°), eye_relief=20.0mm, pupil_diameter=4.0mm, display_sphere_radius=40.0mm, min_fnumber=1.6, pixel_pitch=0.7μm) #these numbers give a reasonable system, albeit with 2000 lenslets. However displays are quite small (184x252)μm
end

"""System parameters which will yield fewer lenslets which will draw much faster"""
function smallsystemparameters()
    return (eye_box=(10.0mm, 9.0mm), fov=(20.0°, 20.0°), eye_relief=20.0mm, pupil_diameter=2mm, display_sphere_radius=40.0mm, min_fnumber=1.6, pixel_pitch=0.7μm) #these numbers give a reasonable system, albeit with 2000 lenslets. However displays are quite small (184x252)μm
end
...
function setup_system(eye_box, fov, eye_relief, pupil_diameter, display_sphere_radius, min_fnumber, pixel_pitch; no_eyebox_subdivision::Bool=false)
    ...
    props = system_properties(eye_relief, eye_box, fov, pupil_diameter, 0.2, 11, pixelpitch=pixel_pitch, minfnumber=min_fnumber, no_eyebox_subdivision=no_eyebox_subdivision)
