...
defaultclusterproperties() = (mtf=0.2, minfnumber=2.0, cyclesperdegree=11, λ=530nm, pixelpitch=0.9μm)
...
compute_focal_length(pixel_pitch::Unitful.Length, pixel_angle) = 0.5 * pixel_pitch / tan(0.5 * pixel_angle)
...
```
"""
function system_properties(eyerelief::Unitful.Length, eyebox::NTuple{2,Unitful.Length}, fov, pupildiameter::Unitful.Length, mtf, cyclesperdegree; minfnumber=2.0, RGB=true, λ=530nm, pixelpitch=0.9μm, maxdisplaysize=250μm, no_eyebox_subdivision::Bool=false)::Dict{Symbol,Any}
...
function printsystemproperties(eyerelief::Unitful.Length, eyebox::NTuple{2,Unitful.Length}, fov, pupildiameter::Unitful.Length, mtf, cyclesperdegree; minfnumber=2.0, RGB=true, λ=530nm, pixelpitch=0.9μm, maxdisplaysize=350μm)
