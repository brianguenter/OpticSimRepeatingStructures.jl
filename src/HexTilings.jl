...
function clustercolors(lattice)
    elements = clusterelements(lattice)
...
function hex3RGB(scale::T=1.0) where {T<:Real}
    ...
    return ClusterWithProperties{RGBCluster}(hex3(scale), properties)
end
export hex3RGB
...
function hex4RGB(scale::T=1.0) where {T<:Real}
    ...
    names = colornames(colors)
    properties = DataFrames.DataFrame(Color=colors, Name=names)
    return ClusterWithProperties{RGBCluster}(hex4(scale), properties)
end
export hex4RGB
...
function hex7RGB(scale::T=1.0) where {T<:Real}
    ...
    names = colornames(colors)
    properties = DataFrames.DataFrame(Color=colors, Name=names)
    return ClusterWithProperties{RGBCluster}(hex7(scale), properties)
end
export hex7RGB
...
function hex9RGB(scale::T=1.0) where {T<:Real}
    ...
    names = colornames(colors)
    properties = DataFrames.DataFrame(Color=colors, Name=names)
    return ClusterWithProperties{RGBCluster}(hex9(scale), properties)
end
export hex9RGB
...
function hex12RGB(scale::T=1.0) where {T<:Real}
    ...
    names = colornames(colors)
    properties = DataFrames.DataFrame(Color=colors, Name=names)
    return ClusterWithProperties{RGBCluster}(lattice, properties)
end
export hex12RGB
